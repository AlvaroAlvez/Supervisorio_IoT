...
class Mqtt_class():
    ...
    def on_publish(self, client,userdata,result): # função de callback, quando o paho publicar recebeemos uma função de callback
    ...
    def on_message(self, client, userdata, msg): #eu não preciso chamar essa função, no paho lib, essa função é chamada automaticamente se chegar uma msg no tópico subscrito
        ...
        with self.payload_lock:#garante que apenas uma thread por vez tente acessar o que está no bloco, no caso self.payload
